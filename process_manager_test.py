...
class ClusterPool:
    def __init__(self, manager ):
        self.manager_list = manager.list()   # インスタンス変数にサーバプロセスで管理するListを保持
    ...
    def __exit__(self, exc_type, exc_value, tb):
        ...
        self.manager_list[:] = []  # withブロックを抜ける時にサーバプロセスのListを空に

    def pop_or_create_cluster(self) -> Cluster:
        
        try:
            return self.manager_list.pop()  # popしてサーバプロセスのListから削除
        except IndexError:
            return Cluster()

    def append_cluster(self, cluster: Cluster):
        self.manager.append(cluster)  # 最後にサーバプロセスのListに追加

def wrap_parallel_func(args) -> int:
    num, cluster_pool = args
    parallel_func(num, cluster_pool)

def parallel_func(num, cluster_pool) -> int:
...
def main():
    ...
    with Manager() as manager:
        with ClusterPool(manager) as cluster_pool:
            job_args = [(num, cluster_pool) for num in nums]
